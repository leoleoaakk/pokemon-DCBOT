...
async function searchAbility(keywords){
    ...
    if( data != null){
        ...
        let URL="https://wiki.52poke.com/zh-hant";
        ...
        if(formChange.length!=0){
            ...
            //index_top=data.search(`<a href="/wiki/`)+15;
            index_top=data.search(`（特性）">`)+6;
            let i=0;
            do{
                /*data=data.substr(index_top);
                index_bottom=data.search(`" title="`);
                URLData[i]=data.substr(0,index_bottom);*/
                data=data.substr(index_top);
